...
def test_authors(metadata):
    ...
    assert metadata["author"] == ["Elnaz Azmi, Martin Laviale"]
...
def test_emails(metadata):
    ...
    assert metadata["author-email"] == {
        "Elnaz Azmi, Martin Laviale": "elnaz.azmi@kit.edu, martin.laviale@univ-lorraine.fr"
    }
...
def test_description(metadata):
    ...
    assert metadata["description"] == "An application to detect and predict diatoms on microscopic images."
